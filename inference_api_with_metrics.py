"""
BLACK VEIL Inference API - WITH METRICS
"""
...
# Import metrics
from prometheus_metrics import track_prediction, track_error, update_models_loaded, REQUEST_COUNT, REQUEST_LATENCY
...
async def load_models():
    ...
    try:
        ...
        # Update metrics
        update_models_loaded(len(MODELS))
...
async def predict(request: PredictionRequest):
    ...
    if n_features is not None and features.shape[1] != n_features:
        track_error('predict', 'feature_mismatch')
        raise HTTPException(
            status_code=400,
    ...
    try:
        ...
        track_prediction(request.model_name, int(prediction))
